...
# code modified from C. Rojas
################################################################################

## CODE FOR: Supplementary Figure 3 generating stacked bar plots of microbial community composition for 
# GALAPAGOS data at the bacterial phylum, order, and genus taxonomic levels
...
#creating paneled plot using Phylum data
paneled <- ggplot(pbar, aes(Sample_Depth, taxa_rel, fill=Taxa))+
  ylab('Relative Abundances (%)') +
  xlab('Sample Depth')+
  geom_bar(stat="identity") +
  facet_grid(~Site_Plant, scales="free_x")+
  facet_wrap(~ Site_Plant,ncol=2)+
...
ggsave(filename="SFig3_galapagos16S_paneled_stacked_phyla.pdf",
       device="pdf",path="./images",
       plot=paneled,
       width=15,
       height=5,
...
##save image 
ggsave(filename="SFig3_Galapagos16S_stacked_order.pdf",
       device="pdf",path="./images",
       plot=barord,
       width=15,
       height=5,
...
##save image 
ggsave(filename="SFig3_Galapagos16S_stacked_gen.pdf",
       device="pdf",path="./images",
       plot=bargen,
       width=15,
       height=8,
